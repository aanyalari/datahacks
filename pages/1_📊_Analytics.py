...
st.caption(f"**{len(df):,}** rows after date filter (sidebar time window already applied).")

tab_phys, tab_bio, tab_acid, tab_ext = st.tabs(["Physical", "Biological", "Acidification", "Extremes"])

with tab_phys:
    st.info(
        "**Physical oceanography:** temperature and salinity trace water masses. "
        "The **T–S diagram** fingerprints parcels — each cluster is water with a shared formation history and different biogeochemical traits."
    )
...
with tab_bio:
    st.info(
        "**Biological pump:** chlorophyll tracks phytoplankton biomass while nitrate fuels new production. "
        "When **Chl rises without NO₃ collapsing**, recycling or physical accumulation may dominate; when **both move together**, you often see new growth."
    )
...
with tab_acid:
    st.info(
        "**Acidification:** drifting pH sensors flag changing hydrogen-ion activity; **Ω_aragonite** links chemistry to "
        "organisms that precipitate aragonite shells. Values near or below **1** mean carbonate undersaturation risk for sensitive calcifiers."
    )
...
with tab_ext:
    st.info(
        "**Extremes & structure:** Hovmöller plots stack depth-resolved SST in one glance; **seasonal radar** compresses how multiple variables "
        "shape together by season — good for judges who will not read every line plot."
    )
